...
def get_mask_subset_with_prob(mask, prob):
    ...
    randperm_indices.masked_fill_(
        randperm_indices < 0, seq_len
    )  # set to max out of bounds, so never chosen
...
class MaskGit(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.continuous_pos_bias = ContinuousPositionBias(
            dim=dim_head, heads=heads, num_dims=3
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        x = x * self.gradient_shrink_alpha + x.detach() * (
            1 - self.gradient_shrink_alpha
        )
...
class SelfCritic(nn.Module):
    def __init__(self, maskgit: MaskGit):
        super().__init__()
        self.maskgit = maskgit

        self.to_pred = nn.Sequential(
            nn.Linear(maskgit.dim, 1), Rearrange("... 1 -> ...")
        )
...
class MaskGITTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        assert (
            not exists(critic)
            or self_token_critic
            or (not maskgit.unconditional) == critic.has_cross_attn
        )
        ...
        self.cond_drop_prob = (
            cond_drop_prob  # classifier free guidance for transformers - @crowsonkb
        )
    ...
    def sample(
        ...
        *,
        ...
        cond_scale=3.0,
        starting_temperature=0.9,
        noise_K=1.0,  # hyperparameter for noising of critic score in section 3.2 of token-critic paper, need to find correct value
    ):
        ...
        if has_prime:
            with torch.no_grad():
                prime_token_ids = self.ctvit(
                    prime_frames, return_only_codebook_ids=True
                )
        ...
        num_tokens = self.ctvit.num_tokens_per_frames(
            num_frames, include_first_frame=not exists(prime_frames)
        )
        ...
        patch_shape = self.ctvit.get_video_patch_shape(
            num_frames + prime_num_frames, include_first_frame=True
        )
        ...
        for step in range(self.steps):
            ...
            if not is_first_step and exists(scores):
                ...
                num_tokens_mask = (
                    (num_tokens * torch.cos(time * math.pi * 0.5))
                    .round()
                    .long()
                    .clamp(min=1)
                )
            ...
            input_token_ids = (
                video_token_ids
                if not has_prime
                else torch.cat((prime_token_ids, video_token_ids), dim=-1)
            )
                ...
                if exists(self.critic):
                    ...
                    with torch.no_grad():
                        ...
                        scores = self.critic.forward_with_cond_scale(
                            critic_input_token_ids, **critic_kwargs
                        )
                    ...
                    noise = (
                        noise_K
                        * (uniform(scores.shape, device) - 0.5)
                        * noise_multiplier
                    )
                    scores = scores + noise
                else:
    ...
    def forward(
        ...
        *,
        ...
        assert not (
            exists(videos) and not exists(self.ctvit)
        ), "ctvit must be provided if one wants to encode the videos live during training"
        assert (
            exists(text_embeds) ^ exists(texts)
        ) ^ self.unconditional, "either raw text of text embeds must be given, and if unconditional, none should be given"

        assert not (
            exists(text_embeds) and text_embeds.shape[-1] != self.text_embed_dim
        ), "text embedding dimension is not correct"
        ...
        if not exists(video_codebook_ids):
            ...
            with torch.no_grad():
                self.ctvit.eval()
                video_codebook_ids = self.ctvit(videos, return_only_codebook_ids=True)
            ...
            if not exists(text_embeds):
                with torch.no_grad():
                    text_embeds = self.encode_texts(
                        texts, output_device=video_codebook_ids.device
                    )

            text_mask = torch.any(
                text_embeds != 0, dim=-1
            )  # save the researcher from having to think about mask, by assuming if all of the feature dimension is 0, it is masked out
        ...
        if exists(video_frame_mask):
            video_mask = self.ctvit.calculate_video_token_mask(
                videos, video_frame_mask=video_frame_mask
            )
        ...
        mask_token_prob = torch.cos(
            rand_step * math.pi * 0.5 / self.steps
        )  # cosine schedule was best
        ...
        if not only_train_critic:
            loss = F.cross_entropy(
                logits[mask_token_mask], video_codebook_ids[mask_token_mask]
            )
        ...
        critic_loss = F.binary_cross_entropy_with_logits(
            pred_fake_or_real_logits, critic_labels
        )
...
@beartype
def make_video(
    maskgittransformer: MaskGITTransformer, texts: List[str], num_frames, prime_lengths
):
    ...
    for text, scene_num_frames, next_scene_prime_length in zip(
        texts, num_frames, prime_lengths
    ):
        video = maskgittransformer.sample(
            texts=text, prime_frames=video_prime, num_frames=scene_num_frames
        )
        scenes.append(video)

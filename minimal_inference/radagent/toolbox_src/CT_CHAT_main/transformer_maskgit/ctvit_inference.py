...
class CustomBatchSampler(BatchSampler):
    ...
    def __len__(self):
        if self.drop_last:
            ...
            return sum(
                -(-len(group) // self.batch_size) for group in self.groups.values()
            )
...
class CTVIT_inf(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.dl = DataLoader(
            self.ds, batch_size=batch_size, shuffle=False, num_workers=8
        )

        self.valid_dl = DataLoader(
            self.valid_ds, batch_size=batch_size, shuffle=False, num_workers=8
        )
    ...
    def infer(self, log_fn=noop):
        ...
        if True:
            ...
            if self.use_ema:
                vaes_to_evaluate = (
                    (self.ema_vae.ema_model, f"{steps}.ema"),
                ) + vaes_to_evaluate

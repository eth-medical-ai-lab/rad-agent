...
class PatchDropout(nn.Module):
    ...
    def forward(self, x, force_keep_all=False):
        if not self.training or self.prob == 0.0 or force_keep_all:
            return x
...
class VisionTransformer(nn.Module):
    def __init__(
        self, dim, *, image_size, patch_size, channels, patch_dropout=0.5, **kwargs
    ):
        super().__init__()
        assert (
...
class CTCLIP(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.text_encode_without_mask = (
            text_encode_without_mask  # whether to pass in text mask to text encoder
        )
        ...
        assert not (
            text_causal_mask and not exists(text_eos_id)
    ...
    def token_embedding(self, input_ids):
        ...
        buffered_token_type_ids = self.text_transformer.embeddings.token_type_ids[
            :, :seq_length
        ]
        ...
        text_embeddings = self.text_transformer.embeddings(
    ...
    def forward(
        ...
        assert not (
            not return_loss and is_multiview
        ), "do not pass in augmented texts or images if not training"
        assert not (
            self.multiview_loss_weight == 0 and is_multiview
        ), "multiview loss weight cannot be 0 if augmented text or images passed in"
        ...
        if self.text_causal_mask:
            ...
            assert torch.all(
                torch.any(eos_text_mask, dim=-1)
            ), f"some of the text rows does not have the eos id {self.text_eos_id}"

            text_len = text.shape[-1]
            eos_indices = eos_text_mask.float().argmax(dim=-1, keepdim=True)

            eos_text_mask = (
                torch.zeros_like(eos_text_mask).scatter(1, eos_indices, 1.0).bool()
            )
            ...
            rest_tokens = rearrange(
                rest_tokens, "(b n d) -> b n d", b=b, n=text_len - 1
            )
        ...
        if self.use_all_token_embeds:
            assert (
                enc_text.ndim == 3
            ), "encoded text must have 3 dimensions (batch, seq, features)"
            assert (
                enc_image.ndim == 3
            ), "encoded image must have 3 dimensions (batch, seq [height x width], features)"
        ...
        if self.extra_latent_projection:
            ...
            text_latents_extra, image_latents_extra = map(
                l2norm, (text_latents_extra, image_latents_extra)
            )
        ...
        text_latents = rearrange(
            text_latents, "(m b) ... -> m b ...", m=num_batch_texts
        )
        image_latents = rearrange(
            image_latents, "(m b) ... -> m b ...", m=num_batch_images
        )

        if self.extra_latent_projection:
            text_latents_extra = rearrange(
                text_latents_extra, "(m b) ... -> m b ...", m=num_batch_texts
            )
            image_latents_extra = rearrange(
                image_latents_extra, "(m b) ... -> m b ...", m=num_batch_images
            )
        ...
        if self.use_all_token_embeds:
            ...
            sim_text_to_image = (
                einsum("m x t d, n y i d -> m n x y t i", text_latents, image_latents)
                * temp
            )
            ...
            text_to_image_mask = rearrange(
                text_mask, "(m b) t -> m 1 b 1 t", m=num_batch_texts
            ).bool()
            text_to_image = masked_mean(text_to_image, text_to_image_mask, dim=-1)

            image_to_text_mask = rearrange(
                text_mask, "(m b) t -> m 1 b 1 t 1", m=num_batch_texts
            ).bool()
            masked_sim = sim_image_to_text.masked_fill(
                ~image_to_text_mask, max_neg_value(sim_image_to_text.dtype)
            )
            image_to_text = reduce(
                reduce(masked_sim, "... t i -> ... i", "max"), "... i -> ...", "mean"
            )
        else:
            text_to_image = (
                einsum("m t d, n i d -> m n t i", text_latents, image_latents) * temp
            )
        ...
        text_to_image_pos, image_to_text_pos = map(
            matrix_diag, (text_to_image_exp, image_to_text_exp)
        )
        ...
        if self.decoupled_contrastive_learning:
            ...
            text_to_image_exp, image_to_text_exp = map(
                lambda t: t.masked_fill(pos_mask, 0.0),
                (text_to_image_exp, image_to_text_exp),
            )

        text_to_image_denom, image_to_text_denom = map(
            lambda t: t.sum(dim=-1), (text_to_image_exp, image_to_text_exp)
        )
        ...
        cl_loss_weight = 1 - (
            self.text_ssl_loss_weight
            + self.image_ssl_loss_weight

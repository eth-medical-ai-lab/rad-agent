...
from evaluation.green_score.utils import (
    make_prompt,
    clean_responses,
...
from distributed_utils import gather_processes, get_rank, is_main_process, tqdm_on_main
...
class GREEN:
    def __init__(
        self, model_name=None, output_dir=".", cpu=False, compute_summary_stats=False
    ):
        super().__init__()
    ...
    def infer(self):
        ...
        if is_distributed and not self.cpu:
        ...
        if is_distributed and not self.cpu:
            self.completions = gather_processes(local_completions)
            self.prompts = gather_processes(local_references)
        else:
    ...
    def process_results(self):
        print("GREEN SCORE starting to process results ...", flush=True)
        self.green_scores = [
            self.compute_green(response) for response in tqdm(self.completions)
        ]
        self.error_counts = pd.DataFrame(
            [self.compute_error_count(response) for response in tqdm(self.completions)],
        ...
        mean, std, summary = np.mean(self.green_scores), np.std(self.green_scores), None

...
class CTChat_full_model(nn.Module):
    ...
    def run_batch_with_messages(self, conversation_trajectories, image_paths):
        batch_size = len(conversation_trajectories)
        image_tensors = []
        prompts = []
        image_sizes = []
        ...
        failed_indices = []
        for i in range(batch_size):
            conversation_trajectory = conversation_trajectories[i]
            image_path = image_paths[i]
            ...
            image = image.to(self.device)
            image_tensors.append(image)
            prompts.append(self.build_prompt_from_messages(conversation_trajectory))
    ...
    def build_prompt_from_messages(self, messages):
        ...
        for idx, message in enumerate(messages):
            ...
            if idx == 0:
                assert role == "user", "CT Chat conversations must start with a user message"
            else:
                prev_role = messages[idx - 1]["role"]
                assert role != prev_role, (
                    "CT Chat message history must strictly alternate between user and assistant"
                )

        conv = conv_templates[self.conv_mode].copy()
        role_map = {"user": conv.roles[0], "assistant": conv.roles[1]}

        for message in messages:
            conv.append_message(role_map[message["role"]], message["content"])
...
if __name__ == "__main__":
    example_nii_path = (
        CT_RATE_ROOT
        / "train_fixed/dataset/train_fixed/train_4/train_4_a/train_4_a_1.nii.gz"
    )

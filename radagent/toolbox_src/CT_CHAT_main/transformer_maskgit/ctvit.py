...
class CTViT(nn.Module):
    ...
    def load(self, path):
        path = Path(path)
        assert path.exists()
        pt = torch.load(str(path), weights_only=True)
    ...
    def encode(self, tokens):
        ...
        self.spatial_rel_pos_bias.net = self.spatial_rel_pos_bias.net.to(tokens.device)
        attn_bias = self.spatial_rel_pos_bias(h, w, device=tokens.device)
        self.enc_spatial_transformer = self.enc_spatial_transformer.to(tokens.device)
        ...
        *_, h, w, _ = shape
        ...
        if video.shape[1] == 1:
            input_vgg_input2, recon_vgg_input2 = map(
                ...
                (input_vgg_input, recon_vgg_input),
            )
        transform = T.Compose([T.Resize(256)])

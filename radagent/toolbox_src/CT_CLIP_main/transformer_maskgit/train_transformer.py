    ...
    def __init__(
        ...
        *,
        ...
        self.ds = VideoTextDataset(
            data_folder="example_data/ctvit-transformer",
            xlsx_file="example_data/data_reports.xlsx",
            num_frames=2,
        )
    ...
    def train_step(self):
        ...
        if exists(self.max_grad_norm):
            self.accelerator.clip_grad_norm_(
                self.maskgittransformer.parameters(), self.max_grad_norm
            )
            ...
            with torch.no_grad():
                ...
                for model, filename in vaes_to_evaluate:
                    ...
                    for tensor in recons.unbind(dim=0):
                        ...
                        tensor_to_nifti(
                            tensor, str(sampled_videos_path / f"{filename}_{i}.nii.gz")
                        )
                        tensor_to_nifti(
                            valid_data[0],
                            str(sampled_videos_path / f"{filename}_{i}_input.nii.gz"),
                        )
                        i = i + 1
